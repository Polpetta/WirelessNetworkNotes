...
\subsubparagraph*{MILD} MILD\footnote{That means \textit{Multiple Increase
  Linear Decrease}} has a more conservative approach than Binary exponential
back off, as it has a slower $cw$ reduction: when a timeout occurs the $cw$ is
multiplied by $1,5$ and when a transmission is successful it get decreased
only by one.
